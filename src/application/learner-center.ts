...
const learnerCenterRowSchema = z.object({
  ...
  content_available_at: z.string().nullable(),
});
...
export async function readLearnerCenterRows(client: SupabaseClient) {
  const { data, error } = await client
    ...
    .select(
      "enrollment_id,course_title,delivery_type,enrollment_status,confirmed_valid_seconds,required_seconds,next_live_starts_at,certificate_status,certificate_id,course_version_id,course_slug,completed_at,has_cover,completion_due_at,content_available_at",
    );
