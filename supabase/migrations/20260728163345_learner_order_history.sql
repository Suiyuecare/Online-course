...
begin
  ...
  if row_limit not between 1 and 50 then
  ...
  with classified as materialized (
    select
      ...
      case
        ...
        ) in ('expired', 'cancelled') then 'closed_refund'
...
create or replace function public.read_own_order_history(
...
as $$
  select internal.read_own_order_history(
    p_category,
    p_limit,
    p_before_created_at,
    p_before_order_id
...
revoke all on function public.read_own_order_history(
  text, integer, timestamptz, uuid
) from public, anon, authenticated, service_role;
